...
void CriticCore::Tick(void)
{
	...
	else if(m_iInfinite == 2)
	{
		...
		{
			m_pOutCriticTarget_v[iSelected] = m_pOutActorTarget_v[iSelected] = *m_pInReinforcement_v;
		}
		...
		copy_array(m_pOutActorTarget_v, m_pInLastActorAction_v, m_iActionLength);

		if(max(m_pInCritic_v, m_iActionLength) > max(m_pInLastCriticAction_v, m_iActionLength))
		{
			m_pOutActorTarget_v[iSelected] = 1;
		}
		else
		{
			float first = max(m_pInLastCriticAction_v, m_iActionLength);
			float second = min(m_pInLastCriticAction_v, m_iActionLength);
			for(int i=0; i<m_iActionLength; ++i)
				if(m_pInLastCriticAction_v[i] < first && m_pInLastCriticAction_v[i] > second)
					second = m_pInLastCriticAction_v[i];

			m_pOutActorTarget_v[iSelected] = second * m_flDiscount;
		}
	}
	else
	{
